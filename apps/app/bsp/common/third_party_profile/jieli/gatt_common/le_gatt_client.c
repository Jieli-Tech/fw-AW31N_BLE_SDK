...
typedef struct {
    ...
    const gatt_client_cfg_t *client_config;//client配置项
    const gatt_search_cfg_t *gatt_search_config;//搜索profile配置项
...
void ble_gatt_client_passkey_input(u32 *key, u16 conn_handle)
{
    ...
    *key = 123456; //default key
    ...
    extern void little_endian_store_32(uint8_t *buffer, uint16_t pos, uint32_t value);
    little_endian_store_32((uint8_t *)tmp_val, 2, (u32)key);
