...
void ble_profile_init(void)
{
    ...
    if (STACK_IS_SUPPORT_GATT_CONNECT()) {
        ...
        log_info("need_ram = %d\n", need_ram);
        if (!gatt_ram_buffer) {
            gatt_ram_buffer = att_send_ram_alloc(need_ram);
            if (!gatt_ram_buffer) {
                log_info("att alloc size=%d fail!!!\n", need_ram);
                ASSERT(0);
                while (1);
            }
 ...
 */
/*************************************************************************************************/
void ble_comm_init(gatt_ctrl_t *control_blk)
{
    int error_reset = 0;
...
void ble_comm_exit(void)
{
    ...
    if (gatt_ram_buffer) {
        ...
        att_send_ram_free(gatt_ram_buffer);
        gatt_ram_buffer = NULL;
    }
}
...
{
    u8 ble_device_name_len = 10; //名字长度，不包含结束符
    char ble_ext_name[] = "(BLE)";
...
int ble_comm_att_send_data(u16 conn_handle, u16 att_handle, u8 *data, u16 len, att_op_type_e op_type)
{
    ...
    case ATT_OP_READ:
        tmp_16  = 0x55A1;//fixed
        ret = (gatt_op_ret_e)ble_op_multi_att_send_data(conn_handle, att_handle, &tmp_16, 2, op_type);
        break;
    ...
    default:
        ret = (gatt_op_ret_e)ble_op_multi_att_send_data(conn_handle, att_handle, data, len, op_type);
        break;
    }
