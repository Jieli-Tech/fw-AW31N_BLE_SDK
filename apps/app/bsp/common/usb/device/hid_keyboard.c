...
void *usb_hid_keyboard_init(void)
{
    ...
    _hid_var.ep_in_buffer = usb_get_ep_buffer(0, HID_KEYBOARD_EP_IN | USB_DIR_IN);
    _hid_var.ep_out_buffer = usb_get_ep_buffer(0, HID_KEYBOARD_EP_OUT | USB_DIR_OUT);
...
static u32 hid_tx_data(struct usb_device_t *usb_device, const u8 *buffer, u32 len)
{
    ...
    return usb_g_intr_write(usb_id, HID_KEYBOARD_EP_IN, buffer, len);
}
...
static void hid_endpoint_init(struct usb_device_t *usb_device, u32 itf)
{
    ...
    usb_g_ep_config(usb_id, HID_KEYBOARD_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_INT, 0, ep_buffer, HID_KEYBOARD_EP_IN_MAX_SIZE);

    ep_buffer = hid_var->ep_out_buffer;
    if (ep_buffer) {
        usb_g_set_intr_hander(usb_id, HID_KEYBOARD_EP_OUT, hid_rx_data);
        usb_g_ep_config(usb_id, HID_KEYBOARD_EP_OUT, USB_ENDPOINT_XFER_INT, 1, ep_buffer, HID_KEYBOARD_EP_IN_MAX_SIZE);
    }
    usb_enable_ep(usb_id, HID_KEYBOARD_EP_OUT);
}
