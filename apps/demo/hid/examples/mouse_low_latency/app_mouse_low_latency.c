/*********************************************************************************************
 *   Filename        : app_mouse_low_latency.c

 *   Description     :低延时模式demo + 2.4G(1K回报率)+ 可加USB模式
...
#define LOG_TAG             "[MOUSE_LOW_LATENCY]"
...
static const char mouse_ble_name[] = "LOW_LATENCY";
...
/*************************************************************************************************/
/*!
 *  \brief      1. 用于做数据接收处理(适用于500us，1ms双连接流程)，不开sm优先级高于ll_conn_rx_acl_hook_get,
 ...
 *  \param
 *
 *  \return     1. false，走ll_conn_rx_acl_hook_get流程;
 *              2. true，走数据处理流程；
 *
 *
 *  \note       1. 中断回调及所加函数中不能添加打印, 会影响cpu执行效率.
...
void ble_event_irq_hook(void *priv_hw, void *link, uint8_t hw_state, uint8_t flag)
{
    if (HID_MODE_BLE == mouse_info.mouse_hid_mode || mouse_info.mouse_is_paired == 0) {
        //printf("%d %d ", mouse_info.mouse_hid_mode, mouse_info.mouse_is_paired);
        return;
    }
