...
static  u8 hid_information1[] = {0x00, 0x00, 0x00, 0x00};
 ...
 */
/*************************************************************************************************/
static void __ble_state_to_user(u8 state, u32 value)
{
...
static int hogp_event_packet_handler(int event, u8 *packet, u16 size, u8 *ext_param)
{
    ...
    case GATT_COMM_EVENT_CONNECTION_UPDATE_REQUEST_RESULT:
        __ble_state_to_user(BLE_ST_CONNECT_UPDATE_REQUEST, 0);
        break;
 ...
 */
/*************************************************************************************************/
static int hogp_make_set_rsp_data(u8 adv_reconnect, u8 *adv_name_ok)
{
    u8 offset = 0;
...
static void __hogp_reconnect_low_timeout_handle(void *priv)
{
...
static void hogp_adv_config_set(void)
{
    ...
    if (PAIR_RECONNECT_ADV_EN && hogp_pair_info.pair_flag && hogp_pair_info.direct_adv_cnt) {
        ...
        if (pair_reconnect_adv_type != ADV_DIRECT_IND) {
            ...
            if (pair_reconnect_adv_timeout) {
                ...
                hogp_adv_timeout_number = sys_timeout_add(NULL, __hogp_reconnect_low_timeout_handle, pair_reconnect_adv_timeout);
    ...
    u8 adv_set_name_ok = 0;
    ret |= hogp_make_set_adv_data(adv_reconnect_flag, &adv_set_name_ok);
    ret |= hogp_make_set_rsp_data(adv_reconnect_flag, &adv_set_name_ok);
    ...
    if (PAIR_RECONNECT_ADV_EN && hogp_pair_info.pair_flag) {
        ...
        if (adv_type == ADV_DIRECT_IND) {
            ...
            if (adv_timeout) {
                ...
                hogp_adv_timeout_number = sys_timeout_add(NULL, __hogp_reconnect_low_timeout_handle, adv_timeout);
            }
        }
    } else {
        memset(hogp_server_adv_config.direct_address_info, 0, 7);
...
#if TCFG_SYS_LVD_EN && HID_BATTERY_TIMER_SET
